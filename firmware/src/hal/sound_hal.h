...
// Play a SHORT note — the reset chime's first ~170 ms and nothing else. Named
// for what it PLAYS rather than for one caller, because it now has three, and
// they mean different things with the same sound:
//
//   the Volume row      judge the level on the spot, not at the next reset
//   the town hall press acknowledge a round that takes ~20 s to show anything
//   an answered card    the agent moved; the thing you asked for happened
//
// The full chime is 2.4 s, far too long to hang off a press and long enough
// that a second press would land on top of the first. Boards with no speaker
// ignore it, so shared code may call it unconditionally.
void sound_hal_play_short(void);
